...
const DB = {
  ...
  _fillMissing() {
    ...
    for (const uid of ['janny','jackie']) {
      if (!this.data.users[uid]) this.data.users[uid] = d.users[uid];
  ...
  todayHomework(id) {
    ...
    if (!this.data.homework[id][t]) this.data.homework[id][t] = { content:'', photo:'', hash:'', completed:false, ts:null };
...
function route() {
  ...
  if (parts[0] === 'mother') {
    ...
    showPage('home');
    Mother.render(parts[1]||'home');
  } else if (parts[0] === 'child') {
    const uid = parts[1];
    if (!['janny','jackie'].includes(uid)) return go('');
    APP.currentUser = uid;
    if (parts.length === 2) {
      // 孩子主页
...
const ChildHome = {
  render(uid) {
    ...
    $$('[data-go="home"]').forEach(b=> b.onclick = () => { APP.currentUser=null; go(''); });
...
async function renderHomework(uid) {
  ...
  const html = `
    ...
    <div class="section-card">
      <div class="section-title">📸 拍照（必须现场拍摄）</div>
      ${hw.photo ? `<img src="${hw.photo}" style="width:100%; border-radius:12px; margin-bottom:10px;" />` :
      ...
      <button class="btn-finish" id="hwTake" style="background:linear-gradient(135deg,#FF8FB1,${p.color})">📸 ${hw.photo?'重新拍照':'现在拍照'}</button>
  ...
  $('#hwTake', sub).onclick = async () => {
    const photo = await takePhoto({ tip:'把作业本拍清楚', uid });
    if (!photo) return;
    hw.photo = photo.dataUrl;
    DB.save();
    toast('✅ 拍照成功');
    renderHomework(uid);
  };
...
/* 录入新错题 */
async function addNewMistake(uid) {
  const p = PROFILES[uid];
  const photo = await takePhoto({ tip:'把错题拍清楚', uid });
  if (!photo) return;
  // OCR识别
  ...
  `).then((res) => {
    ...
    setTimeout(() => {
      ...
      `).then((r) => {
        ...
        const id = uid + '_' + uid();
        if (!DB.data.mistakes[uid]) DB.data.mistakes[uid] = {};
        DB.data.mistakes[uid][id] = {
          question: text,
          photo: photo.dataUrl,
          hash: photo.hash,
          errorReason: reason,
        ...
        DB.save();
        // 推送妈妈检查
        Mother.pushCheck(uid, 'mistake_new', { id });
        toast('✅ 已加入错题本，3天后再复习');
        go(`child/${uid}/mistakes`);
      });
    }, 100);
  });
  $('#ocrImgPreview').innerHTML = `<img src="${photo.dataUrl}" style="width:100%; border-radius:8px;">`;
}
...
async function reviewMistake(uid, mistakeId) {
  ...
  `).then(async (res) => {
    if (!res) return;
    const photo = await takePhoto({ tip:'把重做的题目拍清楚', uid });
    if (!photo) return;
    ...
    `).then(async (result) => {
      ...
      m.attempts.push({ ts: Date.now(), correct: isCorrect, errorReason, photo: photo.dataUrl, hash: photo.hash });
      m.photo = photo.dataUrl;
      m.hash = photo.hash;
      m.round = (m.round||0) + 1;
      ...
      if (isCorrect) {
        ...
        if (u.combo >= 3) {
          // combo特效
          bigEncourage(pick(ENCOURAGE[p.gender].combo), 10);
          addPoints(uid, 10 + 5, '错题连击');
          u.combo = 0;
        } else {
          bigEncourage(pick(ENCOURAGE.generic.correct), 5);
          addPoints(uid, 5, '错题复习正确');
        }
        // 进入下一轮倒计时
        if (m.round >= 4) {
          m.archived = true;
...
/* ===================== 通用：积分动效 ===================== */
function addPoints(uid, n, reason) {
  const u = DB.user(uid);
...
function renderPoemPlan(ct, uid, item, plan) {
  ...
  writeBox.querySelector('#poemShoot').onclick = async () => {
    const photo = await takePhoto({ tip:'把抄写的古诗拍清楚', uid });
    if (photo) {
      writeBox.querySelector('#poemImg').innerHTML = `<img src="${photo.dataUrl}" style="width:100%; border-radius:12px;">`;
      DB.save();
    }
  };
  ...
  if (todayKey >= day4) {
    ...
    mu.querySelector('#poemDictation').onclick = async () => {
      const photo = await takePhoto({ tip:'默写拍清楚', uid });
      if (photo) {
        mu.querySelector('#poemDictationImg').innerHTML = `<img src="${photo.dataUrl}" style="width:100%; border-radius:12px; margin-top:10px;">`;
        DB.save();
...
function renderReadZh(ct, uid, item, plan) {
  ...
  ct.querySelector('#doPractice').onclick = async () => {
    const photo = await takePhoto({ tip:'把练习题拍清楚', uid });
    if (photo) {
      ct.querySelector('#practiceImg').innerHTML = `<img src="${photo.dataUrl}" style="width:100%; border-radius:12px;">`;
      Mother.pushCheck(uid, 'read_zh_practice', { photo: photo.dataUrl });
      DB.save();
      toast('已通知妈妈检查');
    }
  };
...
function renderEssayZh(ct, uid, item, plan) {
  ...
  ct.querySelector('#essayWrite').onclick = async () => {
    const photo = await takePhoto({ tip:'默写拍清楚', uid });
    if (photo) {
      ct.querySelector('#essayImg').innerHTML = `<img src="${photo.dataUrl}" style="width:100%; border-radius:12px; margin-top:10px;">`;
      Mother.pushCheck(uid, 'essay_zh', { photo: photo.dataUrl });
      DB.save();
    }
  };
}
...
function renderCalc(ct, uid, item, plan) {
  ...
  ct.querySelector('#calcShoot').onclick = async () => {
    const photo = await takePhoto({ tip:'把练习页拍清楚', uid });
    if (photo) {
      ct.querySelector('#calcImg').innerHTML = `<img src="${photo.dataUrl}" style="width:100%; border-radius:12px; margin-top:10px;">`;
      Mother.pushCheck(uid, 'math_calc', { photo: photo.dataUrl });
      DB.save();
      toast('已通知妈妈检查');
    }
  };
...
function renderMathThink(ct, uid, item, plan) {
  ...
  ct.querySelector('#mtShoot').onclick = async () => {
    const photo = await takePhoto({ tip:'把练习题拍清楚', uid });
    if (photo) {
      ct.querySelector('#mtImg').innerHTML = `<img src="${photo.dataUrl}" style="width:100%; border-radius:12px; margin-top:10px;">`;
      Mother.pushCheck(uid, 'math_thinking', { photo: photo.dataUrl });
      DB.save();
    }
  };
}
...
function renderEngNote(ct, uid, name, type) {
  ...
  ct.querySelector('#engNoteShoot').onclick = async () => {
    const photo = await takePhoto({ tip:'把笔记拍清楚', uid });
    if (photo) {
      ct.querySelector('#engNoteImg').innerHTML = `<img src="${photo.dataUrl}" style="width:100%; border-radius:12px; margin-top:10px;">`;
      Mother.pushCheck(uid, `${type}_note`, { photo: photo.dataUrl });
      DB.save();
    }
  };
...
function renderSimplePhoto(ct, uid, item, plan) {
  ...
  ct.querySelector('#simShoot').onclick = async () => {
    const photo = await takePhoto({ tip:`拍下${typeName}`, uid });
    if (photo) {
      ct.querySelector('#simImg').innerHTML = `<img src="${photo.dataUrl}" style="width:100%; border-radius:12px; margin-top:10px;">`;
      Mother.pushCheck(uid, plan.type, { photo: photo.dataUrl });
      DB.save();
    }
  };
}
...
function renderBCZ(ct, uid, item, plan) {
  ...
  ct.querySelector('#bczFinish').onclick = () => {
    ...
    ct.querySelector('#bczPhoto').onclick = async () => {
      const photo = await takePhoto({ tip:'把答题拍下来', uid });
      if (photo) {
        ct.querySelector('#bczPhotoArea').innerHTML = `<img src="${photo.dataUrl}" style="width:100%; border-radius:12px; margin-top:10px;">`;
        Mother.pushCheck(uid, 'bcz', { passage, photo: photo.dataUrl });
        DB.save();
        toast('已通知妈妈');
      }
    };
...
function renderTest(ct, uid, item, plan) {
  ...
  ct.querySelector('#testShoot').onclick = async () => {
    const photo = await takePhoto({ tip:'拍下你的卷子', uid });
    if (photo) {
      ct.querySelector('#testImg').innerHTML = `<img src="${photo.dataUrl}" style="width:100%; border-radius:12px; margin-top:10px;">`;
    }
  };
...
function motherTabs(tab, child) {
  ...
  return `
    ...
    <div class="mother-child-tabs">
      <button class="${child==='janny'?'active-girl':''}" data-child="janny">Janny（六年级）</button>
      <button class="${child==='jackie'?'active-boy':''}" data-child="jackie">Jackie（三年级）</button>
    </div>
...
function renderMomPoems(body, child) {
  ...
  body.querySelectorAll('.poem-photo').forEach(btn => {
    btn.onclick = async () => {
      ...
      const prof = PROFILES[kid];
      const photo = await takePhoto({ tip: '拍下给 ' + prof.name + ' 学的古诗' });
      if (photo) {
        const text = await ocrImage(photo.dataUrl);
        if (!DB.data.poems[kid]) DB.data.poems[kid] = [];
        DB.data.poems[kid].push({ text, photo: photo.dataUrl, name: 'OCR识别', ts: Date.now() });
        DB.save();
        toast('已为 ' + prof.name + ' 上传古诗');
...
function sendPenalty(type, message, targetChild) {
  ...
  if (typeof WECHAT_PUSH !== 'undefined' && WECHAT_PUSH) {
    const nameMap = { janny:'Janny', jackie:'Jackie' };
...
function checkDailyReminder() {
  ...
  // 在19:00~19:05范围提示一次
  if (hh === 19 && mm < 5) {
    if (APP.currentUser && ['janny','jackie'].includes(APP.currentUser)) {
